...
void MathLogger::showStats() {
    ...
    std::unordered_map<std::string, std::string> typeNameMap = {
        ...
        {"linear_equation", "Phương trình tuyến tính"},
        {"quadratic_equation", "Phương trình bậc 2"},
        {"system_equation", "Hệ phương trình"}
    };
    std::cout << "📋 Phân loại:\n";
    for (const auto& pair : typeCounts) {
        std::string displayName = typeNameMap.count(pair.first) ? typeNameMap[pair.first] : pair.first;
        std::cout << "   • " << displayName << ": " << pair.second << " bài\n";
    }
