...
// Hàm progress bar đơn giản
static void showSolveProgress(const std::string& filename, int current, int total, 
                               int duplicates, int solved, int failed) {
    const int barWidth = 50;
    float progress = (float)current / total;
    int pos = (int)(barWidth * progress);
    
    // Xóa màn hình và về đầu dòng
...
ImportStats DataImporter::batchSolve(const std::vector<std::string>& problems, bool skipDuplicates, const std::string& filename) {
    ...
    math::Calculator calc(logger_, true);  // true = silent mode
    ...
    for (size_t i = 0; i < problems.size(); i++) {
        ...
        // processedCount++;
        // std::cout << "  [" << processedCount << "/" << problems.size() << "] " 
        //           << problem << " ... ";
        ...
        std::string displayName = filename.empty() ? "Unknown" : filename;
        showSolveProgress(displayName, i + 1, problems.size(), stats.duplicates, stats.solved, stats.failed);
    }
