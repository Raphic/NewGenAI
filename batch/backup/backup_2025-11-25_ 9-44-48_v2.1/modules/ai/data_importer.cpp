...
static void showSolveProgress(int current, int total, int errors, int skipped) {
    const int barWidth = 50;
    float progress = (float)current / total;
    int pos = (int)(barWidth * progress);
    
    std::cout << "\r📝 Đang giải: [";
    ...
    std::cout << "] " << (int)(progress * 100) << "% (" 
              << current << "/" << total << ")";
    if (errors > 0) std::cout << " ❌" << errors;
    if (skipped > 0) std::cout << " ⏭️" << skipped;
    std::cout << std::flush;
}
...
ImportStats DataImporter::importAndSolveCSV(const std::string& csvFile, bool skipDuplicates) {
    ...
    // GỌI batchSolve
    return batchSolve(problems, skipDuplicates);
}
...
ImportStats DataImporter::batchSolve(const std::vector<std::string>& problems, bool skipDuplicates) {
    ImportStats stats = {0, 0, 0, 0, 0};
    ...
    for (size_t i = 0; i < problems.size(); i++) {
        ...
        showSolveProgress(i + 1, problems.size(), stats.failed, stats.duplicates);
    }
