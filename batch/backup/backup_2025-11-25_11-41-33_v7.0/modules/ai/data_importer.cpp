    ...
    if (choice == "1") {
        ...
        for (const auto& f : files) {
            if (!f.imported) {
                ...
                if (f.fullPath.find(".json") != std::string::npos) {
                    ...
                    std::cout << "📦 JSON file - Đang merge...\n\n";
                    // Progress bar giả (vì append rất nhanh)
                    std::cout << f.fullPath << "\n";
                    std::cout << "[██████████████████████████████████████████████████] 100% (" << f.problemCount << "/" << f.problemCount << ")\n";
                    std::cout << "📝 Merge trực tiếp (không giải lại)" << std::flush;
                    ...
                    if (src.is_open() && dst.is_open()) {
                        ...
                        std::cout << "\n\n✅ Đã merge " << f.problemCount << " bài vào math_history.json\n";
                    ...
                    // CSV/TXT - Check trước
                    auto existing = getExistingProblems();
                    
                    std::vector<std::string> fileProblems;
                    if (f.fullPath.find(".csv") != std::string::npos) {
                        auto data = readCSV(f.fullPath);
                        for (const auto& td : data) {
                            fileProblems.push_back(td.problem);
                        }
                    } else {
                        fileProblems = readTextFile(f.fullPath);
                    }
                    
                    // Đếm bài mới
                    int newCount = 0;
                    for (const auto& p : fileProblems) {
                        std::string trimmed = p;
                        trimmed.erase(0, trimmed.find_first_not_of(" \t\r\n"));
                        trimmed.erase(trimmed.find_last_not_of(" \t\r\n") + 1);
                        if (!trimmed.empty() && trimmed[0] != '#') {
                            if (existing.find(normalizeForComparison(trimmed)) == existing.end()) {
                                newCount++;
                            }
                        }
                    }
                    ...
                    if (newCount == 0) {
                        // Toàn bộ đã có
                        std::cout << "📥 Kiểm tra file...\n";
                        std::cout << "✅ File đã import đầy đủ (" << fileProblems.size() << " bài)\n";
                        std::cout << "⏭️  Bỏ qua toàn bộ (không có bài mới)\n\n";
                        stats = {0, 0, 0, 0, 0};
                    } else {
                        // Có bài mới
                        std::cout << "📊 Tổng: " << fileProblems.size() << " bài | ";
                        std::cout << "⏭️  Đã có: " << (fileProblems.size() - newCount) << " | ";
                        std::cout << "🆕 Cần học: " << newCount << "\n\n";
        ...
        while (iss >> idx) {
            if (idx > 0 && idx <= (int)files.size()) {
                ...
                ImportStats stats;
                if (f.fullPath.find(".json") != std::string::npos) {
                    // JSON - Check đã merge chưa
                    if (f.imported) {
                        std::cout << "📦 JSON file - Đã merge trước đó\n";
                        std::cout << "✅ Bỏ qua để tránh duplicate\n\n";
                        stats = {0, 0, 0, 0, 0};
                    } else {
                        // Chưa merge → merge mới
                        std::cout << "📦 JSON file - Đang merge...\n\n";
                        std::cout << f.fullPath << "\n";
                        std::cout << "[██████████████████████████████████████████████████] 100% (" << f.problemCount << "/" << f.problemCount << ")\n";
                        std::cout << "📝 Merge trực tiếp (không giải lại)" << std::flush;
                        ...
                        if (src.is_open() && dst.is_open()) {
                            ...
                            dst.close();
                            hasImported = true;
                            std::cout << "\n\n✅ Đã merge " << f.problemCount << " bài\n";
                            std::cout << "💡 Gõ 'cleanup' để xóa bài trùng\n";
                        } else {
                            std::cout << "\n❌ Lỗi\n";
                        }
                        stats = {0, 0, 0, 0, 0};
